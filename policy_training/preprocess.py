...
FOLDER_NAME = "0812_pick_yoghurt_out_from_fridge_venky"
DATA_PATH = Path(DATA_DIR) / "teleop_data"
...
# TODO: We probs don't need task names
task_names = [FOLDER_NAME]
...
for TASK_NAME in task_names:
    ...
    if Path(f"{SAVE_PATH}/{TASK_NAME}").exists():
        print(
            f"Previous demonstrations from task {TASK_NAME} exist. Appending new demonstrations to the existing ones."
        )
        num_prev_demos = len(
    ...
    for num, demo_dir in enumerate(sorted(DEMO_DIRS)):
            ...
            for cam in cam_indices:
                ...
                if os.path.exists(full_folder_path):
                    ...
                    output_video_path = os.path.join(
                        base_folder_path, f"camera{cam}.mp4"
                    )
                    cmd = [
                        "ffmpeg",
                        "-f",
        ...
        #     print("Error processing demonstration", demo_dir.name)
