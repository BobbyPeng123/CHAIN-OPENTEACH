...
class Actor(nn.Module):
    def __init__(
        ...
        if policy_head == "deterministic":
        ...
        elif policy_head == "discrete":
            self._action_head = DiscreteHead(
                hidden_dim, self._act_dim, hidden_size=hidden_dim, num_layers=2
            )
            ...
            self._action_head = DiffusionHead(
                ...
                obs_horizon=10,  # 3 (dmc - diffusion), #10, 1 (calvin)
                pred_horizon=10,  # 3 (dmc - diffusion), #10, 1 (calvin)
                hidden_size=hidden_dim,
                num_layers=2,  # 1,  # 2,
                device=device,
            )
    ...
    def forward(self, obs, num_prompt_feats, stddev, action=None, cluster_centers=None):
        ...
        # # Last step only
        # features = features[:, -1:]
        # action = action[:, -1:] if action is not None else None
...
class BCAgent:
    def __init__(
        ...
        self.use_aux_inputs = use_aux_inputs if obs_type == "pixels" else False
        self.norm = norm
        self.train_encoder = train_encoder
        self.history_len = history_len if history else {"default": 1}
        self.eval_history_len = eval_history_len if history else {"default": 1}
        self.separate_encoders = separate_encoders
        self.aux_encoders = aux_history_encoders if history else aux_encoders
        ...
        #     self.ema_update_every = (82 * 50 * 200) // (
        #         16**2 * 10
        #     )  # 82 tasks, 50 demos, 200 steps per demo, bs 16
        ...
        if use_aux_inputs:
            ...
            for key in self.aux_keys:
                if (
                    self.aux_encoders is not None
                    and utils.history_key_map(key) in self.aux_encoders
                ):
                    self.aux_projector[key] = self.aux_encoders[
                        utils.history_key_map(key)
                    ](aux_shape[key]).to(device)
                else:
        ...
        if obs_type == "pixels" and policy_type == "mlp":
            ...
            if use_aux_inputs:
                new_repr_dim += self.repr_dim * len(self.aux_keys)
            if self.use_language:
        ...
        if self.use_language:
            ...
            # self.language_scheduler = torch.optim.lr_scheduler.StepLR(
            #     self.language_opt, step_size=15000, gamma=0.1
            # )
        # action projector
        if self.use_actions:
    ...
    def train(self, training=True):
        ...
        if training:
            ...
            if self.obs_type == "pixels" and self.use_aux_inputs:
    ...
    def buffer_reset(self):
        if self.obs_type == "pixels":
            ...
            for key in self.pixel_keys:
                try:
                    self.observation_buffer[key] = deque(
                        maxlen=self.eval_history_len[utils.history_key_map(key)]
                    )
                except KeyError:
                    self.eval_history_len[
                        utils.history_key_map(key)
                    ] = self.eval_history_len["default"]
                    self.observation_buffer[key] = deque(
                        maxlen=self.eval_history_len[utils.history_key_map(key)]
                    )
            ...
            if self.use_aux_inputs:
                ...
                for key in self.aux_keys:
                    try:
                        self.aux_buffer[key] = deque(maxlen=self.eval_history_len[key])
                    except KeyError:
                        self.aux_buffer[key] = deque(
                            maxlen=self.eval_history_len["default"]
                        )
                    # self.aux_buffer[key] = deque(maxlen=self.eval_history_len)
        else:
    ...
    def discretize(self, actions):
        ...
        for action in actions:
            action = np.lib.stride_tricks.sliding_window_view(
                # actions[0], (self.num_queries, action.shape[-1])
                action,
        ...
        if self.policy_head == "bet":
            actions = torch.as_tensor(reshaped_actions, device=self.device).float()
    ...
    def act(self, obs, prompt, norm_stats, step, global_step, eval_mode=False):
        ...
        # lang projection
        # TODO: Not compatible with new history lengths
        if self.use_language:
            ...
            repeat_len = (
                ...
                if self.obs_type == "pixels"
                else len(self.observation_buffer)
            )
            lang_features = (
        ...
        if self.obs_type == "pixels":
            ...
            for key in self.pixel_keys:
                ...
                # encoder
                pixels = (
                    self.encoder[key](pixels, lang=lang_features)
                    if self.separate_encoders
                    else self.encoder(pixels, lang=lang_features)
                )
            ...
            if self.use_aux_inputs:
                for key in self.aux_keys:
                    ...
                    features.append(aux_feat[0])

            # if self.use_language:
            #     features.append(lang_features)
            # features.append(self.action_token.unsqueeze(0).repeat(repeat_len, 1))
            features = torch.cat(features, dim=-1).view(-1, self.repr_dim)
        else:
            self.observation_buffer.append(pre_process(obs[self.feature_key]))
        ...
        #                 prompt[f"prompt_{self.aux_keys}"], device=self.device
        #             ).float()
        ...
        action = self.actor(features.unsqueeze(0), 0, stddev, **kwargs)
    ...
    def update(self, expert_replay_iter, step):
        ...
        if self.obs_type == "pixels":
            ...
            for key in self.pixel_keys:
                ...
                if self.train_encoder:
                    pixel = (
                        self.encoder[key](pixel, lang=lang_features)
                        if self.separate_encoders
                        else self.encoder(pixel, lang=lang_features)
                    )
                else:
                    with torch.no_grad():
                        pixel = (
        ...
        #             prompt_features.append(pixel)
        #         if self.use_aux_inputs:
        #             proprio = data[f"prompt_{self.aux_keys}"].float()
        #             proprio = self.proprio_projector(proprio)
        #             prompt_features.append(proprio)
        #         if self.use_actions:
        ...
        _, actor_loss = self.actor(features, 0, stddev, action, **kwargs)
        if self.train_encoder:
