...
class RobotEnv(gym.Env):
    ...
    def reset(self, seed=None):  # currently same positions, with gripper opening
        if self.use_robot:
            ...
            try:
                sensor_state = np.array(ret["sensor_state"]["reskin"]["sensor_values"])
            ...
            if not self.separate_sensors:
                obs["sensor"] = np.zeros(self.sensor_dim * (1 + self.use_sensor_diffs))
                self.sensor_prev_state = obs["sensor"]
            else:
                for sensor_idx in range(self.n_sensors):
                    obs[f"sensor{sensor_idx}"] = np.zeros(
                        self.per_sensor_dim * (1 + self.use_sensor_diffs)
                    )
